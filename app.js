...
/* jshint strict: false */
/* global App */
App.controller('PageCtrl', ['$scope', '$title', function ($scope, $title) { $scope.$root.title = $title; }]);
...
App.controller('OrganiseCtrl', ['$scope', '$routeParams', '$title', 'idaTasks', 'idaEvents', function ($scope, $routeParams, $title, $tasks, $events) {
  ...
  if ($routeParams.breakdown) {
    $scope.breakdown = [$tasks.get($routeParams.breakdown)];
    if ($scope.breakdown[0]) {
      $scope.breakdown[0].showChildren = true;
      $scope.breakdown[0].expanded = true;
    }
  }
}]);
...
/* jshint strict: false */
/* global App, moment, _ */
App.controller('StatisticsCtrl', ['$scope', '$title', 'idaTasks', 'idaEvents', function($scope, $title, $tasks, $events){
  ...
  $scope.refresh = function () {
    ...
    $scope.stats = [
      { name: 'Klara A', value: 0 },
      { name: 'Klara B', value: 0 },
      { name: 'Klara AA', value: 0 },
      { name: 'Klara BB', value: 0 },
      { name: 'Klara Oplanerade', value: 0 },
      { name: 'Påminnelser', value: _.where(events, { type: 'reminderSet'}).length },
      { name: 'Nyinlagda', value: _.where(events, { type: 'taskCreated'}).length },
      { name: 'Gjorda planeringar', value: _.where(events, { type: 'taskPlanned'}).length },
      { name: 'Missade', value: _.where(tasks, { overdue: true }).length },
      { name: 'Raderade', value: 0 }
    ];

    _.each(tasks, function(task) {
      if (task.deleted) { $scope.stats[9].value++; }
      if (task.finished) {
        if (task.planned) {
          if (task.important) {
            if (task.complex) { $scope.stats[2].value++; }
            else { $scope.stats[0].value++; }
          } else {
            if (task.complex) { $scope.stats[3].value++; }
            else { $scope.stats[1].value++; }
          }
        } else {
          $scope.stats[4].value++;
        }
...
/* jshint strict: false */
/* global App*/
App.controller('TodoCtrl', ['$scope', '$route', '$title', 'idaTasks', function($scope, $route, $title, $tasks){
...
App.directive('idaHold', function () {
  return {
    link: function ($scope, element, attrs) {
      var start, move, end, x1, x2, y1, y2, holding;
      ...
      start = _.throttle(function (e) {
        ...
        _.delay(function (e) {
          if (!holding) { return e; }
          $scope.$apply(attrs.idaHold);
          holding = false;
        }, 500);
      }, 30);
      move = _.throttle(function (e){
        if(!holding) { return e; }
        if (e.clientX < x1 || e.clientX > x2 || e.clientY < y1 || e.clientY > y2) { holding = false; }
      }, 30);
      end = _.throttle(function () { holding = false; }, 30);
...
App.service('idaEvents', function () {
  ...
  // Add an event
  Events.prototype.add = function (type) {
    this.events.push({type: type, time: Date.now()});
  };

  Events.prototype.getWithin = function (start, end) {
    return _.filter(this.events, function(task){
      return (task.endTime || task.startTime) >= start && (task.endTime || task.startTime) <= end;
    });
  };
...
App.service('idaTasks', ['$window', '$timeout', 'idaEvents', 'idaConfig', function ($window, $timeout, $events, $config) {
  ...
  var _tasks;
    ...
    this.focussed = false;
    this.showInTodo = 'until marked as done';
    this.showInTodoUntil = Infinity;
  ...
  Task.prototype.postpone = function(days) {
    this.startTime = moment().day(moment().day() + days)._d.valueOf();
  ...
  Task.prototype.updatePeriodInput = function () {
    ...
    switch(this.timeInputType) {
      case 'today':
        ...
        this.endTime = moment().hour(23).minute(59)._d.valueOf();
        break;
      case 'x-days':
        this.xDays = this.xDays || 1;
        this.startTime = Date.now().valueOf();
        this.endTime = moment().day(moment().day() + this.xDays)._d.valueOf();
        break;
      case 'tomorrow':
        this.startTime = moment().hour(24).minute(0)._d.valueOf();
        this.endTime = moment().hour(48).minute(-1)._d.valueOf();
        break;
      case 'week':
        this.startTime = Date.now().valueOf();
        this.endTime = moment().startOf('week').day(7).hour(23).minute(59)._d.valueOf();
        break;
      case 'next-week':
        this.startTime = moment().startOf('week').day(8).hour(0).minute(0)._d.valueOf();
        this.endTime = moment().startOf('week').day(14).hour(23).minute(59)._d.valueOf();
        break;
      case 'month':
        this.startTime = Date.now().valueOf();
        this.endTime = moment().month(moment().month() + 1).date(0).hour(23).minute(59)._d.valueOf();
        break;
      case 'next-month':
        this.startTime = moment().month(moment().month() + 1).date(1).hour(0).minute(0)._d.valueOf();
        this.endTime = moment().month(moment().month() + 2).date(0).hour(23).minute(59)._d.valueOf();
        break;
      case 'three-months':
        this.startTime = Date.now().valueOf();
        this.endTime = moment().month(moment().month() + 3).hour(23).minute(59)._d.valueOf();
        break;
    }
  };
  ...
  // findWhere
  Tasks.prototype.findWhere = function(obj) { return _.findWhere(this.tasks, obj); };
  ...
  Tasks.prototype.showChildren = function (shown) { _.each(this.tasks, function (task) { task.showChildren = shown; }); };
  ...
  // Change focused task
	Tasks.prototype.focus = function (id) {
    (_.findWhere(this.tasks, { focussed: true }) || {}).focussed = false;
    (_.findWhere(this.tasks, { id: id }) || {}).focussed = true;
  };

  // Generate distraction list
  Tasks.prototype.distractionListTasks = function (loadFocusTime) {
    if(!loadFocusTime) { return []; }
    return this.tasks.filter(function(x) {
      return !x.deleted && !x.finished && !x.focussed && !x.planned && x.timeCreated > loadFocusTime;
    });
  };
  ...
  Tasks.prototype.moveFinishedTasksToArchive = function () {
    this.tasks.filter(function(x) { return x.checked; }).forEach(function(x) {
      x.finished = true;
      x.checked = false;
    });
    this.save();
  ...
  Tasks.prototype.detachChild = function (id) {
    _.each(this.tasks, function(task) {
      if(task.children.indexOf(id) !== -1) {
        task.children = _.without(task.children, id);
  ...
  Tasks.prototype.getTodoList = function (todoFilter) {
    var now = new Date().valueOf(), _this = this;
    return _.sortBy(_.filter(_this.tasks, function(task){
      var shift = $config.pragmaticDayshift,
          hrs = {
            'today': 24 + shift,
            'tomorrow': 48 + shift,
            'week': 168 + shift
      ...
      var value = 0, shift = $config.pragmaticDayshift;
      if (task.timeType === 'exact') {
        value += 3000000 - (task.startTime % 2630000000) / 10000;
      } else if (
        task.important && (
          task.endTime < moment().hour(1 * 24 + shift)._d.valueOf() || (
            task.complex && (
              (
                !task.isParent && $scope.$parent.todoFilter === 'week' && (
                  task.endTime < moment().hour(2 * 24 + shift)._d.valueOf() ||
                  (task.endTime < moment().hour(3 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 7 * 86400000) ||
                  (task.endTime < moment().hour(5 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 15 * 86400000) ||
                  (task.endTime < moment().hour(7 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 22 * 86400000) ||
                  (task.endTime < moment().hour(9 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 28 * 86400000)
                )
              ) || (
                (task.endTime < moment().hour(2 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 3 * 86400000) ||
                (task.endTime < moment().hour(3 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 7 * 86400000) ||
                (task.endTime < moment().hour(4 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 14 * 86400000) ||
                (task.endTime < moment().hour(5 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 20 * 86400000)
              )
            )
          ) || (
            (task.endTime < moment().hour(2 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 4 * 86400000) ||
            (task.endTime < moment().hour(3 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 9 * 86400000) ||
            (task.endTime < moment().hour(4 * 24 + shift)._d.valueOf() && task.endTime - task.startTime > 25 * 86400000)
          )
        )
      ) { value += 3000000; }
      else { value += 1000000; }
  ...
  Tasks.prototype.getTimePeriods = function () {
    ...
    (function(){
      var calc, now = moment().startOf('day')._d.valueOf();

      calc = _.reduce(_this.tasks, function(memo, task){
        if (task.deleted || task.finished || task.timeType !== 'exact' || task.startTime < now || !task.planned) { return memo; }
        var duration = task.duration || $config.defaultDuration,
            d1 = task.timeEstimated ? duration * 0.5 : duration,
            d2 = task.timeEstimated ? duration * 1.5 : duration;
        memo.blocks.push([task.startTime, task.startTime + d1]);
        memo.blocks.push([task.startTime, task.startTime + d2]);
        memo.cuts.push(task.startTime, task.startTime + d1, task.startTime + d2);
        return memo;
      }, { blocks: [], cuts: [] });
      calc.cuts = _.uniq(calc.cuts).sort();
    ...
    while(stop > time._d.valueOf()) {
      if(time.year() !== year) {
        year = time.year();
        curYear = periods.push({
          time: time._d.valueOf(),
          start: time._d.valueOf(),
          end: new Date(time.year() + 1, 0, 1, 0, 0, 0, 0).valueOf(),
          depth: 0,
      ...
      curDay = periods.push({
        ...
        time: time._d.valueOf(),
        depth: 2,
        label: time.format('ddd D MMM'),
        isWeekend: time.day() === 6 || time.day() === 0,
        busyness: (function(){
          var dayOp,
              rp,
              ut = 0,
              s = $config.slope,
              rt = $config.readjustmentTime,
              vp = $config.daysBeforeDoubleVaugnessPunishment,
              allDayTaskOpSum = _.reduce(
                _.filter(_this.tasks, function(task){
                  return (task.deleted === false && task.planned === true && task.finished === false &&
                    ((
                      task.timeType === 'exact' && task.startTime >= time._d.valueOf() && task.startTime < (time._d.valueOf() + 86400000)
                    ) || (
                      task.timeType === 'period' && (time._d.valueOf() + 86400000) > task.startTime && time._d.valueOf() < task.endTime
                    ))
                  );
                }),
                function(memo, task){
                  ...
                  if(task.timeType === 'period') {
                    ...
                    var x = Math.ceil((time._d.valueOf() - task.startTime) / 86400000),
          ...
          rp = ut * rt/60;
          dayOp = (allDayTaskOpSum + rp) / (1440 * $config.percentageUsableTime);
